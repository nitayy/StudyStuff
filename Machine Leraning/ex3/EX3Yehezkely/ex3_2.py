...
def q1():
    ...
    plt.xlabel("PC #")
    plt.ylabel("Variance")
    plt.plot(pc, pca.explained_variance_ratio_ * 100, 'g')
    plt.show()
...
def q3():
    # Train a SVM classification model
    print("######### Q3 ########")
    print("Q3:  Fitting the classifier to the training set")
    t0 = time()

    # I put the best values to save time:
    param_grid = {'C': [1e3],
                  'gamma': [0.0001], }
    clf = GridSearchCV(SVC(kernel='rbf', class_weight='balanced'), param_grid)
    ...
    print("Q3:    Predicting people's names on the test set")
    t0 = time()
